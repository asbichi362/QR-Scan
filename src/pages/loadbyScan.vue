<template>
  <q-page class="row  justify-center ">
    <q-toolbar class=" q-mb-4 text-white fixed-top">
    ...
    <q-card   class=" col-xs-10 bg-transparent   rounded-borders col-md-4 col-xs-11 col-sm-11 q-mt-xl">
      <qrcode-stream style="border-radius: 24px; height: 70%;"  class="q-mt-lg" :camera="camera" @decode="onDecode" @init="onInit">
        <div v-if="validationSuccess" class="validation-success rounded-borders">
          This is a URL
        </div>
        <!--if result is returned sell ticket-->

        <div v-if="dialog.result!=false" class=" validation-failure  rounded-borders">
          <div class="desert-bg rounded-borders">
            <q-card class="full-height   glass-top-dark rounded-borders q-pa-md">
              ...
              <q-card-section>
                <q-item-label class="text-white text-center"  >Ticket Varified <br><q-icon size="md" class="text-center" color="white" name="security" /></q-item-label>
      ...
      <!--      geolocation-->
      <q-card-actions  class="fixed-bottom shadow-2 q-pa-md  q-mt-md " align="center">
        <q-btn outline  :to="{ name: 'loadbyCode' }" icon="list"  color="green" label="Type"  />
        <q-btn outline @click="turnCameraOn" icon="videocam"  color="green" label="On" />
        <q-btn outline @click="turnCameraOff" icon="videocam_off"  color="green" label="Off" />
      </q-card-actions>
    </q-card>
    <!--    success dialog-->
    <!--    not success dialog-->
  ...
  export default {
    ...
    methods: {
      ...
      async onDecode (content) {
        ...
        //if gps is not grabbed show prompt
        ...
        // GPS is grabbed do scan and upload
          ...
          const query = 'getSale.php?id=' + content
